...
def predict():
    ...
    # ===== Tambahan: Kirim notifikasi ke backend Hapi.js =====
    try:
        notify_url = "http://localhost:5000/notifications/broadcast"  # Ganti dengan URL endpoint notifikasi Hapi
        payload = {
            'symbol': symbol,
        ...
        response = requests.post(notify_url, json=payload, headers=headers, timeout=5)
        if response.status_code != 200:
            print(f"⚠️ Notifikasi ke Hapi gagal, status: {response.status_code}")
    except Exception as e:
        print(f"❌ Error saat kirim notifikasi ke Hapi: {e}")
    # ==========================================================
